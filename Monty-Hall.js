...
function initializeGame() {
    ...
    doorContents = ['throphy', 'retry', 'lose'];
...
function handleDoorClick(doorIndex) {
    ...
    setTimeout(() => {
        const availableDoors = [0, 1, 2].filter(
            d => d !== doorIndex && doorContents[d] !== 'throphy'
        );
...
function handleDecision(switchDoor) {
    ...
    if (result === 'retry') {
        ...
        // Update status
        if (won) {
            status.innerHTML = `🏆 You won the throphy! ${didSwitch ? '(Switched)' : '(Stayed)'}`;
